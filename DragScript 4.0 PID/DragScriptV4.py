...
[0, 0.796638,3.09445,6.83227,12.1446,19.0946,28.2083,39.7859,52.7009,67.1758,84.0781,102.727,123.357,134.042]]#DUNCAN GOT HIS SHIT TOGETHER :D
    ...
    def velocity_step(hi, vi, mi, ti):
        if vi != 0:
            ...
            sign = 1
        return vi +(-g - sign*drag_curve(hi, vi, ti)*drag_f/mi + thrust_function(ti)/mi)*time_step
...
def PID_test(thrust_profile, rocket_mass, motor_mass, propellant_mass, time_res, temp, burn_time, drag_f, max_deploy, t_start, t_deploy, plots, k_height, k_vel, k_acc, scaling_f, max_ADAS_depl_speed):
    
    nom_time, nom_vel, nom_acc, nom_height, nom_mass, nom_ADAS, nom_apogee, nom_MECO_vel = num_solver(thrust_profile, rocket_mass, motor_mass, propellant_mass, time_res, temp, burn_time, drag_f, max_deploy, t_start, t_deploy, 0)
...
[0, 0.796638,3.09445,6.83227,12.1446,19.0946,28.2083,39.7859,52.7009,67.1758,84.0781,102.727,123.357,134.042]]#DUNCAN GOT HIS SHIT TOGETHER :D
    ...
    def drag_curve(hi, vi, t): #a function that return the drag force on the rocket but also calculates the deployment necessary using PID
        ...
        if t>1:
            cur_acc = (v_arr[index]-v_arr[index-1])/time_step
        else:
            cur_acc = 0
            
        signal_h = hi-nom_height[index] #get index of the current location
        signal_v = vi-nom_vel[index]    #get index of the curr loc
        signal_a = cur_acc-nom_acc[index]
        
        PID_alt_signal.append(signal_h)
        PID_vel_signal.append(signal_v)
        PID_acc_signal.append(signal_a)
        
        PID_total_signal.append((signal_h*k_height + signal_v*k_vel + signal_a*k_acc)*scaling_f)
        
        deployment = 0
        
        if t>burn_time: #dont wanna deploy before the end of teh burn time
            wanted_depl = (signal_h*k_height + signal_v*k_vel + signal_a*k_acc)*scaling_f
    ...
    if plots:
        ...
        plot(t_arr[0:-1], accel_array, '-', color = 'black', label = 'Acceleration')
        plot(nom_time[0:-1], nom_acc, '--', color = 'tomato', label = 'Nominal Acceleration')
        #plot(t_arr[c], accel_array[c], 'o', color = 'black', label = 'Main Engine Cutoff')
        #plot(t_arr[d], accel_array[d], 'o', color = 'tomato', label = 'Apogee')
        xlabel('Time [s]')
        ylabel('Acceleration [m/sec^2]')
        ...
        figure(figsize=(10, 15))
        subplot(3,1,1)
        plot(t_arr[0:-2], PID_alt_signal[0:-1], '-', color = 'black', label = 'PID Altitude Signal')
        plot(t_arr[0:-2], PID_vel_signal[0:-1], '-', color = 'blue', label = 'PID Velcotiy Signal')
        plot(t_arr[0:-2], PID_acc_signal[0:-1], '-', color = 'red', label = 'PID Acceleration Signal')
        xlabel('Time [s]')
        ylabel('Signal')
        title('PID Signals')
        grid()

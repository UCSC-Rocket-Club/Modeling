...
def num_solver(thrust_profile, rocket_mass, motor_mass, propellant_mass, time_res, temp, burn_time, max_deploy, t_start, t_deploy, plots, mass_f):
    ...
    rocket_mass = rocket_mass*mass_f
...
[0, 0.796638,3.09445,6.83227,12.1446,19.0946,28.2083,39.7859,52.7009,67.1758,84.0781,102.727,123.357,134.042]]#DUNCAN GOT HIS SHIT TOGETHER :D
    ...
    def velocity_step(hi, vi, mi, ti):
        if vi != 0:
            ...
            sign = 1
        return vi +(-g - sign * drag_curve(hi, vi, ti)/mi + thrust_function(ti)/mi)*time_step

...
def num_solver(thrust_profile, rocket_mass, motor_mass, propellant_mass, time_res, temp, burn_time, plots):
    ...
    drag_forces = [0, 0.59182, 2.2296, 4.83375, 8.55161, 13.4116, 20.0177, 28.9221]
    drag_speed = [0, 20, 40, 60, 80, 100, 120, 140]
    
    drag_force_function = interp1d(drag_speed, drag_forces, kind = 'quadratic')
    ...
    def velocity_step(hi, vi, mi, ti):
        ...
        if vi<0:
            vi = 0
        return vi + (-g - drag_force_function(vi)*(air_pressure(hi, rho)/(1.225*mi)) + thrust_function(ti)/mi)*time_step
